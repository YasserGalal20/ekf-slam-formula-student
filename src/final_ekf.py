...
class callbacks_data:
  def __init__(self): 
    ...
    rospy.Subscriber("/centroid_lidar_cones_marker",MarkerArray,callback=self.cones_callback)   #subscribes to lidar_detections
...
def search_correspond_LM_ID(xAug, PAug, zi):
    ...
    for i in range(nLM):
        ...
        mdist.append(math.sqrt(np.dot(np.dot(y.T , np.linalg.inv(S)) , y)[0,0]))
...
def jacob_motion(x, call_backs_data):
    ...
    G = np.eye(len(x)) + np.dot(np.dot(Fx.T , jF) ,Fx)
    return G, Fx
...
def observation(Call_backs_data):
    ...
    for i in range(len(Call_backs_data.landmark_x)): # Test all beacons, only add the ones we can see (within MAX_RANGE)
...
def predict(xEst, PEst,call_backs_data):
    ...
    """
    S = STATE_SIZE
    G, Fx = jacob_motion(xEst, call_backs_data)
    xEst[0:S] = motion_model(xEst[0:S],call_backs_data)

    # Fx maps the pose block into the full state
    # sigma = G*sigma*G.T + Noise
    PEst = np.dot( G , np.dot(PEst , G.T )) + np.dot( Fx.T , np.dot(Cx , Fx) )
...
if __name__ == '__main__':
  ...
  R = np.diag([0.25, np.deg2rad(5.0)]) ** 2 # Measurement Noise
  map_x2=[]
  map_y2=[]
